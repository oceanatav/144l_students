...
In which data on individual bottle TOC/DOC data from ACIDD remineralization experiments were processed, quality controlled, and analyzed. 
...
```{r}
excel_sheets("~/Documents/github_144l/144l_students/Input_Data/week4/ACIDD_Exp_TOC.xlsx")

metadata <- read_excel("~/Documents/github_144l/144l_students/Input_Data/week4/ACIDD_Exp_TOC.xlsx", sheet = "Metadata")

data <- read_excel("~/Documents/github_144l/144l_students/Input_Data/week4/ACIDD_Exp_TOC.xlsx", sheet = "Data")
```
...
#PTOC = parallel TOC--the incubation vials
#ash171 =  SD
# ash172 = Santa Barbara channel. We only have data for ash172
```
...
```{r}
custom.colors <- c("Control" = "#9eb0a3", "Ash Leachate" = "#ebb688", "Santa Barbara" = "#55a3a2", "San Diego" = "#bec46a", "Vial" = "#e0e067", "Bottle" = "#8797fa" )
...
```
The analytical detection limit (the sensitivity of our methods) for our TOC analysis is -1.5 umol C L^-1^.

#TOC v PTOC (bottle vs. vial)

Using the Model II regression (both x and y variables are not independent, neither are controlled, both are measured). 
```{r}
...
reg.data %>% 
  ...
  theme_bw() +
  annotate( geom = "text", label = expression(atop("y = 0.72x + 19.8", paste("r"^2, " = 0.97,", italic(" p "), "= 0.01"))), x = 70, y = 74, size = 4) #This text does NOT adjust to upstream changes. x =, y =  gives location for text on graph. ATOP() says which text is on top
```
```{r}
...
library(zoo)
```
Interpolate only w/in an experiemnt, not across experiments. We will apply interpolation by usinf the split-apply-combine strategy.

Split our dataframe into a list of its elements, apply our interpolation function to each list element then recombine into new dataframe. 
...
keys <- to_interpolate %>% 
  group_keys() %>% 
  mutate(key = paste(Experiment, Bottle)) #add names to list elements
