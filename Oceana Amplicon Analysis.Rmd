---
title: "Phyloseq Amplicon Analysis"
...
```{r install phyloseq, message=FALSE}
...
Merge all 3 datasets (taxonomy, ASV by sequence, sample names)
Want to make sure all sample names btw counts of sequences and sample names are the same (this is where errors in creating [hyloseq obj] typically come from)
...
```{r filter sequences for bacterial only}
sub_ps <- ps %>% 
...
```{r plot sequencing depth, echo=FALSE, warning = FALSE, message=FALSE}

# Make df with column for read counts of each sample.

sample_sum_df <- data.frame(sum = sample_sums(sub_ps))

# View histogram of sample read counts
ggplot(sample_sum_df, aes(x = sum)) +
  geom_histogram(color = "black", fill = "#fc9003", binwidth = 3000 )+
  ggtitle("Distribution of sample sequencing depth") +
  xlab("Read counts") +
  theme(axis.title.y = element_blank()) +
  theme_bw()

# Gaps in sampling depth (not continuous)
```

```{r picture of read counts}
...
Calculate matrices such as dissimilarity; using pairwise comparisons of sample. (Relating a sample to another ie beta diversity, which compares community comp.)

**Differences in sampling depth influence the distance/dissimilarity metrics** therefore we need to normalize read depth across out samples.

## Subsample

We will rarefy (randomly subsample with replacement) the read depth of the samples first (scaling to the smallest library size)
...
Unconstrained ordniantions for exploratory analyses of amplicon data. (Using NMDS plot)

Important to set a seed since ordination process is random--allows repitition! 
...
```{r NMDS non-sub-sampled, fig.width=10, fig.height=5}
...
```{r plot NMDS subsampled data, fig.width=12, fig.height=5}
...
```{r merge metadata onto richness}

alphadiv <- left_join(richness, sample.tab %>% 
                        rownames_to_column(., var = "DNA_ID"))
```

```{r visualize alpha diversity metrics, fig.width=11}
...
alpha.plot
```
No p<0.05 exists for comparisons via Krushkal-Wallis. Evenness and diversity did not change significantly

```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 13, fig.align = "center"}
...
That said, we an generate a heat map of our samples showing us how the relative abundance of different taxonomic groups change...potentially giving us a visual of which taxa are most important to the alpha and beta diversity patterns we observed. 
First, we're going to generate a custom table that will be easier to work with than a phyloseq object.c
...
#install.packages("viridis") # coloring package
library(viridis)
